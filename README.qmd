...
If we are using Trypsin in our Mass Spec protocol, we'll expect this to be split at either lysine (K) or arginine (R), except when followed by a proline (P). So, these are the peptide sequences we might expect to get in our MS results:
...
Next, we will add some modification metadata and format the same way it is output from Spectronaut.
...
This is typical of what we would see in Spectronaut output. The `parse_mods` function in this package will parse these strings and return a data.frame with the peptide sequences and metadata.
